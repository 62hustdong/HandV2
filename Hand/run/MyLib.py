...
def TimChieuRongNgonTay(mang_diem, img, nguong):
    dinh_ngon_tro, chan_ngon_tro, mang_diem_moc_6 = mang_diem
    image = img.copy()
    ...
    a1, b1, c1 = TimPhuongTrinhDuongThangVuongGocVoiDuongThang(dinh_ngon_tro, chan_ngon_tro, mang_diem_moc_6)
    x = mang_diem_moc_6[0]
    while True:
        y = round((- c1 - a1 * x) / b1)
        (B, G, R) = image[y, x]
...
def TimDiemSo_12_53_54_51_52(mang_diem, img, nguong):
    ...
    chi_so_diem_53 = contour_x.index(max(contour_x))
...
def TimDiemSo_38_39_49_50_tay_phai(mang_diem, img, nguong, handType):
    ...
    if handType == 'Right':
        ...
        while True:
            (B, G, R) = image[y, x]
            if (B, G, R) == (240, 0, 159) and abs(x - diem_so_38[0]) > 10:
                diem_so_39 = (x, y)
                break
            else:
                x = x + 1
    ...
    if handType == 'Left':
        chi_so_diem_39 = contour_x.index(min(contour_x))
        diem_so_39 = contour[chi_so_diem_39]
        ...
        while True:
            (B, G, R) = image[y, x]
            if (B, G, R) == (240, 0, 159) and abs(x - diem_so_39[0]) > 10:
                diem_so_38 = (x, y)
                break
            else:
                x = x - 1
        ...
        chi_so_diem_49 = contour_y_1.index(max(contour_y_1))
        diem_so_49 = contour[chi_so_diem_49]

        chi_so_diem_50 = contour_y_2.index(max(contour_y_2))
        diem_so_50 = contour[chi_so_diem_50]
...
def TimDiem_41_42_43_44_dong(mang_diem, image, handType, nguong):
    mang_diem_moc_5, mang_diem_moc_6, mang_diem_moc_20 = mang_diem

    image_crop = []
    local_coord_crop = []
    if handType == 'Right':
        x_min = mang_diem_moc_6[0]
...
def TimDiem_41_42_43_44_dong_V1(mang_diem, image, handType, nguong):
    ...
    while True:
        ...
        if (B, G, R) == (240, 0, 159) and abs(x_43 - trung_diem_d0_d1[0]) >= 5:
    ...
    while True:
        ...
        if (B, G, R) == (240, 0, 159) and abs(x_44 - trung_diem_d0_d1[0]) >= 5:
    ...
    while True:
        ...
        if (B, G, R) == (240, 0, 159) and abs(x_41 - trung_diem_d2_d3[0]) >= 5:
    ...
    while True:
        ...
        if (B, G, R) == (240, 0, 159) and abs(x_42 - trung_diem_d2_d3[0]) >= 5:
...
def TimDiem_30_31_32_33_34_dong(mang_diem, img, handType, nguong):
    mang_diem_moc_0, mang_diem_moc_3, diem_cao_nhi, diem_cao_nhat = mang_diem
    image = img.copy()
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    gray = cv2.GaussianBlur(gray, (3, 3), 0)
    thresh = cv2.threshold(gray, nguong, 255, cv2.THRESH_BINARY)[1]
    ...
    if handType == 'Right':
        ...
        for point_no, point in enumerate(contour):
            if point[1] <= mang_diem_moc_3[1]:
                contour_x_1[point_no] = w
            if point[1] > mang_diem_moc_0[1]:
                contour_x_1[point_no] = w

        chi_so_diem_35 = contour_x.index(min(contour_x))
        ...
        while True:
            ...
            if (B, G, R) == (240, 0, 159) and abs(x_34 - diem_so_35[0]) >= 10:
        ...
        # Tìm diểm 32
        y_32 = mang_diem_moc_3[1]
        x_32 = mang_diem_moc_3[0]
        while True:
            (B, G, R) = image[y_32, x_32]
            if (B, G, R) == (240, 0, 159) and abs(x_32 - mang_diem_moc_3[0]) >= 10:
                diem_so_32 = (x_32, y_32)
                break
            else:
                x_32 = x_32 + 1

        # Tìm diểm 33
        y_33 = mang_diem_moc_3[1]
        x_33 = mang_diem_moc_3[0]
        while True:
            (B, G, R) = image[y_33, x_33]
            if (B, G, R) == (240, 0, 159) and abs(x_33 - mang_diem_moc_3[0]) >= 10:
        ...
        x_30 = diem_cao_nhi[0]
        while True:
            y_30 = round((- c3 - a3 * x_30) / b3)
            (B, G, R) = image[y_30, x_30]
            if (B, G, R) == (240, 0, 159) and abs(x_30 - diem_cao_nhi[0]) >= 10:
        ...
        x_31 = diem_cao_nhi[0]
        while True:
            y_31 = round((- c3 - a3 * x_31) / b3)
            (B, G, R) = image[y_31, x_31]
            if (B, G, R) == (240, 0, 159) and abs(x_31 - diem_cao_nhi[0]) >= 10:
    ...
    if handType == 'Left':
        ...
        chi_so_diem_35 = contour_x.index(max(contour_x))
        diem_so_35 = contour[chi_so_diem_35]
